...
После `/start` бот показывает приветствие и предлагает кнопку
**📸 Добавить трофей**. Пользователю не нужно выбирать рыбу или человека:
бот делает это автоматически.

## Render: порт и режимы

Для Render Web Service бот поднимает `/health` на переменной `PORT`.
В production рекомендуется `BOT_MODE=webhook`. Если в Render временно стоит
`BOT_MODE=polling`, health-сервер всё равно держит порт открытым.

Кнопка `/start`: **📸 Добавить трофей**.
